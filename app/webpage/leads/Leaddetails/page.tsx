    ...
    <div style={container}>
      <div style={headerRow}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={() => router.push("/webpage/components/EmployeeList")} style={backBtn}>
            ← Back
          </button>
        </div>
        ...
        <>
          <div style={rowStyle}>
            ...
            <div style={rightCardStyle}>
              ...
              {(lead.Contacts || []).map((c: any, idx) => {
  // prefer the table/api fields first, then fallback to older names
  const title = c.ContactTitle ?? c.Title ?? "";
  const role = c.ContactRoleName ?? c.Role ?? "";
  ...
  return (
    <div
      ...
      style={{
        padding: "10px 0",
        borderBottom:
          idx < (lead.Contacts || []).length - 1 ? "1px solid #eee" : "none",
      }}
    >
      {/* Name */}
      ...
      {/* Role */}
      {role ? (
        <div style={{ fontSize: 13, color: "#666", marginTop: 4 }}>{role}</div>
      ) : null}
      ...
      {c.ContactEmail && (
        <div style={{ marginTop: 6 }}>
          <a
            ...
            style={{ color: "#0952d3", textDecoration: "none" }}
          >
            {c.ContactEmail}
          </a>
        </div>
